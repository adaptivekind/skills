...
def git_repo(tmp_path):
    ...
    subprocess.run(['git', 'config', '--local', 'user.email', 'test@example.com'], cwd=tmp_path, check=True, capture_output=True)
    subprocess.run(['git', 'config', '--local', 'user.name', 'Test User'], cwd=tmp_path, check=True, capture_output=True)
    subprocess.run(['git', 'config', '--local', 'commit.gpgsign', 'false'], cwd=tmp_path, check=True, capture_output=True)
    yield tmp_path
...
class TestGit:
    ...
    def test_is_detached_head_after_checkout(self, git, git_repo):
        ...
        subprocess.run(['git', 'add', '.'], cwd=git_repo, check=True, capture_output=True)
        subprocess.run(['git', 'commit', '-m', 'init'], cwd=git_repo, check=True, capture_output=True)
        subprocess.run(['git', 'commit', '--allow-empty', '-m', 'second'], cwd=git_repo, check=True, capture_output=True)
