...
def git_repo(tmp_path):
    ...
    script_path = os.path.join(os.path.dirname(__file__), '..', 'scripts', 'pre-commit.py')
...
class TestPreCommit:
    def test_passes_when_gpg_signing_configured(self, git_repo):
        ...
        subprocess.run(['git', 'config', 'user.signingkey', '12345678'], cwd=git_repo, check=True, capture_output=True)
    ...
    def test_creates_skill_branch_when_on_main_with_skills_changes(self, git_repo):
        ...
        branch = subprocess.run(['git', 'branch', '--show-current'], cwd=git_repo, capture_output=True, text=True).stdout.strip()
    ...
    def test_creates_branch_on_master(self, git_repo):
        ...
        subprocess.run(['git', 'add', '.'], cwd=git_repo, check=True, capture_output=True)
        subprocess.run(['git', 'commit', '-m', 'Initial commit'], cwd=git_repo, check=True, capture_output=True)
        subprocess.run(['git', 'branch', '-m', 'master'], cwd=git_repo, check=True, capture_output=True)
    ...
    def test_does_nothing_when_not_on_main_master_and_has_changes(self, git_repo):
        ...
        subprocess.run(['git', 'checkout', '-b', 'feature/existing-branch'], cwd=git_repo, check=True, capture_output=True)
    ...
    def test_handles_detached_head_state(self, git_repo):
        ...
        subprocess.run(['git', 'checkout', 'HEAD~1'], cwd=git_repo, check=True, capture_output=True)
